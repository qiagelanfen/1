...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.xianxing1 = nn.Linear(self.piny.shape[1], self.configs.c_out)
        self.xianxing2 = nn.Linear(self.piny.shape[0], self.configs.batch_size * (self.pred_len + self.seq_len))
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        lm=self.lm
        gate = torch.sigmoid(self.add(self.T_model(x.permute(0, 2, 1)).permute(0, 2, 1, 3), self.C_model(x)))
        fused_features = gate * self.T_model(x.permute(0, 2, 1)).permute(0, 2, 1, 3) + (1 - gate) * self.C_model(x)

        x = 0.3*fused_features+0.3*self.z_score_normalize(pi)+0.4*lm
