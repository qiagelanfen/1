...
class Exp_Long_Term_Forecast(Exp_Basic):
    ...
    def test(self, setting,test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if test_data.scale and self.args.inverse:
                    shape = outputs.shape
                    print(outputs.shape)
                    # Reshape outputs for inverse transform
        ...
        print('test shape:', preds.shape, trues.shape)
        # trues1,preds1=LMST.lmst_train(pre_len)
        # preds+=preds1
        # trues+=trues1
        # preds/=2
        # trues/=2
        ...
        mae, mse,summ, rmse, mape, mspe = metric(preds, trues)
        print('mse:{}, mae:{}, sum:{},dtw:{}'.format(mse, mae,summ, dtw))
        f = open("./out/results/result_long_term_forecast.txt", 'a')
        f.write(setting + "  \n")
        f.write('mse:{}, mae:{}, sum:{},dtw:{}'.format(mse, mae,summ, dtw))
        f.write('\n')
        f.write('\n')
        f.close()
